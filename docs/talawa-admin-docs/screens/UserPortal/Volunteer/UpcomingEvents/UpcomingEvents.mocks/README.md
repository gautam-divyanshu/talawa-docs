[**talawa-admin**](../../../../../README.md)

***

# screens/UserPortal/Volunteer/UpcomingEvents/UpcomingEvents.mocks

## Variables

- [CREATE\_ERROR\_MOCKS](variables/CREATE_ERROR_MOCKS.md)
- [EMPTY\_MOCKS](variables/EMPTY_MOCKS.md)
- [ERROR\_MOCKS](variables/ERROR_MOCKS.md)
- [MEMBERSHIP\_LOOKUP\_MOCKS](variables/MEMBERSHIP_LOOKUP_MOCKS.md)
- [MEMBERSHIP\_STATUS\_MOCKS](variables/MEMBERSHIP_STATUS_MOCKS.md)
- [MOCKS](variables/MOCKS.md)
- [RECURRING\_MODAL\_MOCKS](variables/RECURRING_MODAL_MOCKS.md)

## References

### baseRecurringEvent

Re-exports [baseRecurringEvent](../UpcomingEvents.mockEvents/variables/baseRecurringEvent.md)
